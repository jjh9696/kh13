...
//		link.attr("href","http://192.168.30.45:8080/member/login");//강의장에서만
		...
		String url = ServletUriComponentsBuilder.fromCurrentContextPath()
												...
												.build().toUriString();
		link.attr("href", url);
//		link.attr("href", ServletUriComponentsBuilder.fromCurrentContextPath()); 한 번에 쓰려면 이렇게 가능
	...
	public void sendTempPassword(MemberDto memberDto) {
		...
		for(int i = 0; i<3; i++) {			
			int pos = r.nextInt(upper.length())+0;//upper에서의 랜덤위치
			buffer.append(upper.charAt(pos));//버퍼에 추가
		}
		for(int i = 0; i<3; i++) {			
			int pos = r.nextInt(number.length())+0;//number에서의 랜덤위치
			buffer.append(number.charAt(pos));//버퍼에 추가
		}
		for(int i = 0; i<3; i++) {			
			int pos = r.nextInt(special.length())+0;//special에서의 랜덤위치
	...
	public void sendCert(String memberEmail) {
		...
		DecimalFormat fmt = new DecimalFormat("000000");//맨앞에 0이 나와도 생략되지 않게 처리
