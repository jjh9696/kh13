...
public class Pay3Controller {
	...
	//-데이터베이스는 payment_detail 과 payment 조회 및 변경이 필요
	...
	public String cancelItem(@RequestParam int paymentDetailNo) throws URISyntaxException {
		...
		// 이거 안해도 db에 제약조건으로 막혀있어서 에러 뜨긴 함
//		if(paymentRemain < amount) {//잔여 금액이 더 작은경우
...
//		}
		...
		return "redirect:detail?paymentNo="+paymentNo; //새로고침 할 때 취소가 계속 되지 않게 리다이렉트로 처리
	}
